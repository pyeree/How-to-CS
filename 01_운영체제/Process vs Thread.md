...
**단점** : 각각 독립된 메모리를 갖고 있어 작업량이 많을수록 오버헤드 발생, [[PCB & Context Switcing|Context Switching]]으로 인한 성능 저하
...
>
> 문제점: 프로세스는 독립된 메모리 영역을 할당받으므로, [[캐시 메모리(Cache Memory)|캐시 메모리]] 초기화와 같은 무거운 작업이 진행되면 오버헤드가 발생할 수 있음
