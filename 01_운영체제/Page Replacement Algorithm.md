...
[[Memory|가상 메모리]]는 `요구 페이지 기법`을 통해 필요한 페이지만 메모리에 적재하고 사용하지 않는 부분은 그대로 둠
...
1. ##### FIFO 알고리즘
   ...
   `초기화 코드` : 처음 [[Process vs Thread|프로세스]] 실행될 때 최초 초기화를 시키는 역할만 진행하고 다른 역할은 수행하지 않으므로, 메인 메모리에서 빼도 괜찮음
