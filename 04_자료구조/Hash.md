...
>
> 하드디스크나, 클라우드에 존재하는 무한한 데이터들을 유한한 개수의 해시값으로 매핑하면 작은 메모리로도 [[Process vs Thread|프로세스]] 관리가 가능해짐!
...
임의의 길이 데이터를 **고정된 길이의 해시 값으로 매핑**하는 것. 해시 함수로 데이터 값을 해시 값으로 바꾸고, 그 값을 index 삼아 데이터를 찾는다.

**쓰는 이유는 적은 자원으로 많은 데이터를 관리하기 위해서다** — 무한한 데이터를 유한한 개수의 해시값으로 매핑하면 작은 메모리로도 관리가 가능해진다. 언제나 동일한 해시값을 리턴하므로 index만 알면 검색이 빠르고, **시간복잡도는 O(1)** 이다 (이진탐색트리는 O(logN)).

문제는 데이터가 많아지면 서로 다른 데이터가 같은 해시 값을 갖는 **collision(충돌)** 이 생긴다는 것.

| 충돌 해결 | 방식 |
| --- | --- |
| **체이닝** | 연결리스트로 노드를 계속 추가 (제한 없이 연결 가능, but 메모리 문제) |
| **Open Addressing** | 해시 함수로 얻은 주소가 아닌 다른 주소에 저장 허용 (차 있으면 다음 주소로) |
| **선형 탐사** | 정해진 고정 폭으로 옮겨 중복을 피함 |
| **제곱 탐사** | 고정 폭을 제곱수로 옮겨 중복을 피함 |

버킷이 크면 충돌 빈도는 낮아지지만 메모리는 한정된 자원이라 무작정 키울 수 없다. 그래서 **load factor**(할당된 키 개수 / 해시 버킷 크기)가 보통 **0.7~0.8** 이상이면 버킷을 동적 확장하고, 이때 기존 값들을 다시 해싱해 새 키를 부여하는 **리해싱(Rehashing)** 을 거친다.
