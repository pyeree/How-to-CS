...
5. **30초 요약을 대신 써주지 마라.** `🎤 면접 30초 요약`(MANUAL 블록)은 `daily.py`의 완료 판정 근거다 — Claude가 채우면 cron이 도장 찍던 것과 똑같이 대시보드가 거짓말을 한다. 노트에 들어가는 문장은 **사용자가 말한 내용**이어야 한다. "요약 채워줘"라고 해도 바로 쓰지 말고 [[🤖 Claude 학습 루프]] ①의 리콜 세션(설명시킴→구멍 지적→본인 표현으로 정리)으로 진행한다. 사용자가 끝내 못 떠올린 내용은 요약에 넣지 말고 따로 알린다.
