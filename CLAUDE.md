...
6. **본문 밖 심화 지식은 숨기지 말고 알려주되, 정식 요약과는 분리한다.** 리콜 중 노트 본문에 없는 실무·꼬리질문 수준 지식(예: TIME_WAIT의 진짜 이유)이 나오면 대화에서 바로 알려준다. 다만 `🎤 면접 30초 요약`(완료 판정 근거, 본문 밖 사실 금지)에는 섞지 않고, 같은 MANUAL 블록 안에 `### 🔎 더 알아두면 좋은 것 (본문엔 없는 심화 — 요약 채점 대상 아님)` 소제목으로 따로 남긴다. 예시: `02_네트워크/TCP 3 way handshake & 4 way handshake.md`.
7. **오늘 뭐 볼지 추천할 때는 노트 제목만 던지지 말고 항상 전체 경로를 같이 준다.** 예: "OSI 7 계층" (X) → "`02_네트워크/OSI 7 계층.md`" (O). 매번 "그거 어디 있는데"를 되묻게 하면 안 된다.
